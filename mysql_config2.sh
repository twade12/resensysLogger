...
echo "INSERT INTO \`$user\`.\`Accounts\` (\`UserName\`,\`SID\`,\`SIDName\`,\`AccountType\`,\`LastLogin\`,\`Enable\`,\`Password\`,\`LastUpdate\`,\`Email1\`,\`Email2\`,\`Email3\`,\`latitude\`,\`longtitude\`) VALUES('$user','$sid','$sid','Administrator',UTC_TIMESTAMP(),1,'$pw',UTC_TIMESTAMP(),'','','',0,0);" >> siteCommands.sql
echo "GRANT SELECT ON \`Data_$1\`.* to \`$user\`@\`%\` WITH GRANT OPTION;" >> siteCommands.sql
